   ...
   for ([int] $c = 0; $c -lt $containers.Count; $c++)
   {
    ...
    do {
     ...
     if ($blobs -ne $null)
     {
      ...
      {
       if ($blobs[$b].ICloudBlob.Properties.StandardBlobTier -ne "Cool")
       {
    ...
    if ($total_usage -gt 0)
    {
     Write-Host "Found blobs not in cool tier in container $container. Total Size = $total_usage "
    }
   }
  }
